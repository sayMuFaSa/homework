...
int find_in_file(const char *path, const char *pattern);
...
void find_in_dir(const char *path, const char *pattern)
{
	...
	while (fgets(file_path, PATH_MAX, pipe) != 0){
		*strchr(file_path, '\n') = 0;
		int number = find_in_file(file_path, pattern);

		if (number > 0) {
			std::cout << file_path << ": " << number << std::endl;
		}
